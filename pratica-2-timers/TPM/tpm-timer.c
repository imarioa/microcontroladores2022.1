...
int main(void)
{
	...
	SIM->SCGC6 = (1 << 24) | (1 << 25);
	MCG->SC |= (1 << 1) | (1 << 2); //Dividindo o clock por 8 - Novo clock 1Mhz

	TPM_REG->sc |= (1 << 6); //ativando a interrupção para TOF
	//Prescaler 64 - MOD de 1s = 3D09
	TPM_REG->mod = 0x7A11; //valor para 1s;

	TPM_REG->cnt = 0x0000; // resetar

	TPM_REG->sc |= 0x6; //ativado o prescaler para 64
	// mod = (t/(1/f)) - 1
	// mod = (1/(1/(1Mhz/64))) - 1 -> (1/(1*(64*10^(-6)))) -- Para 1s
	// t = 1/(8Mhz/64) = 16us
	TPM_REG->sc |= (1 << 3); //incrementa a cada pulso do LPTPM

	/*****************************************************************************/

	SIM->SCGC5 = (1 << 12) | (1 << 13); //ativar clock porta E e D
